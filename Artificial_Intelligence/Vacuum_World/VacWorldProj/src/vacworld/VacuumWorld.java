...
public class VacuumWorld extends Environment {
    ...
    /**
     * Adds a new agent to environment. Since "vacuum cleaning world" is a 
     * single agent environment, this method should only be called once per
     * object.
     * @param agent, an object Agent
     */
    @Override
    ...
    /**
     * Simulation is complete when the robot has performed its ShutOff action.
     * @return true, if agent is turned off or time is out.
     */
    @Override
    protected boolean isComplete() {
        return ((VacuumState) state).isRobotOff() || timedOut();
    }

    protected boolean timedOut() {
        return !interactive && getNumActions() >= MAX_ACTIONS;
    }

    /**
     * Creates a percept for agent. This implements the see: E -> Per function.
     * @param a, Agent object.
     * @return a Percet object (VacPercept).
     */
    @Override
    ...
    protected void updateState(Agent a, Action action) {
        ...
        if (((VacuumState) state).bumped()) numBumps++;
    ...
    public void Start(State initState) {
        ...
        if (interactive == true) waitForUser();
        ...
        while (!isComplete() && !quit) {
            ...
            if (interactive == true) quit = waitForUser();  
        }
     ...
     * @return true, if the user has chosen to quit the simulation.
     */
            ...
            switch (args[i]) {
                ...
                case "-rand":
                    ...
                    try {
                        ...
                        System.err.println("ERROR: Seed passed with '-rand' must"
                                + " be an integer: " + ex.toString());
                        System.exit(1);
                    }   
                    break;
                default:
    ...
    /**
     * @return the number of actions the agent has executed.
     */
    ...
    /**
     * @return the performance measure of an agent team in the current
    ...
    protected int getPerformanceMeasure() {
        int score = 1000 + getMovesScore() + getTurnsScore() + getSucksScore()
                + getBumpsScore() + getDirtScore();
        
        /*System.out.println(score + ", " + getMovesScore() + ", " 
                + getTurnsScore() + ", " + getSucksScore() + ", "
                + getBumpsScore() + ", " + getDirtScore());*/
        
        if (timedOut()) score = score - 100;
        if (score < 0) score = 0;
          
        return score;
    }

    protected int getMovesScore() {return numMoves * -2;}
    protected int getTurnsScore() {return numTurns * -1;}
    protected int getSucksScore() {return numSucks * -2;}
    protected int getBumpsScore() {return numBumps * -10;}
    protected int getDirtScore() {return getNumDirtyLocs() * -100;}
    ...
    protected void printScore(PrintStream out) {
        ...
        out.printf("in %s actions%n", getNumActions());
        out.println("----------");
        out.println("  Base score: \t1000");
        // out.println("  Home penalty: " + getHomeScore());
        totalScore = getPerformanceMeasure(agents.get(0));
