...
public class GameFrame extends JFrame {
    ...
    private final static String ALPHABETA_PLAYER_STRING = "alfa-beta";
     ...
     */
    private final TicTacToeBoard boardState;
     ...
     *
     * @param type1 Player1's type.
     * @param type2 Player2's type.
     */
    ...
    /**
     * Returns true if the position is a legal one. A legal position is one in
     * which the row takes a value of 0, 1, or 2 and the column takes the value
     * of 0, 1, or 2.
     *
    ...
    private boolean isLegalPosition(Position p) {
        return (p != null) && (p.row >= 0) && (p.row < TicTacToeBoard.SIZE)
    ...
    private void displayBoard(TicTacToeBoard state) {
        for (int row = 0; row < TicTacToeBoard.SIZE; row++) {
            for (int col = 0; col < TicTacToeBoard.SIZE; col++) {
                if (state.getState(row, col).equals(TicTacToeBoard.X))
                    cellLabel[row * TicTacToeBoard.SIZE + col].setText("X");
                else if (state.getState(row, col).equals(TicTacToeBoard.O))
    ...
    public static void main(String[] args) {
        try {
            ...
            if (args.length != 2) {
                ...
                System.out.println("\twhere player type = human, random, minimax, or alphabeta");
                System.exit(-1);
            }
            ...
            } else if (args[0].equals(ALPHABETA_PLAYER_STRING)){
                player1Type = Player.ALPHABETA_PLAYER;
            } else {
            ...
            } else if(args[1].equals(ALPHABETA_PLAYER_STRING)){
                player2Type = Player.ALPHABETA_PLAYER;
            } else {
                throw new Exception("Unrecognized player type");
            }
